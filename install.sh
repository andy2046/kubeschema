...
PROJECT_NAME="kubeschema"
PROJECT_GH="andy2046/$PROJECT_NAME"

: ${HELM_PLUGIN_PATH:="$(helm home)/plugins/kubeschema"}

# Convert the HELM_PLUGIN_PATH to unix if cygpath is available.
...
# initArch discovers the architecture for the system.
initArch() {
  ARCH=$(uname -m)
  case $ARCH in
    armv5*) ARCH="armv5";;
...
# initOS discovers the operating system for the system.
initOS() {
...
# verifySupported checks if the os/arch combination is supported for binary builds.
verifySupported() {
...
installFile() {
  ...
  HELM_TMP_BIN="$HELM_TMP/kubeschema"
